...
public class RegistroFuncionEspectaculo extends JInternalFrame {
	...
	public RegistroFuncionEspectaculo(IControladorFuncion icon) {
		...
		scrollPane2.setBounds(35, 235, 204, 202);
		...
		btnR.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try { 
					...
					JOptionPane.showMessageDialog(btnR, e1.getMessage(), "alta funcion", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		btnR.setBounds(280, 415, 100, 22);
		...
		btnCancelar.setBounds(399, 415, 100, 22);
	...
	private void seleccionarEspectador() {
		String u = (String) cBEspectador.getSelectedItem();
	...
	private void registrar() throws Exception {				
		...
		if (rS.size() != 3 && !rS.isEmpty())
			throw new Exception ("Debe seleccionar 3 registros");
		else if (rS.size() == 3 || rS.isEmpty()) {	
        	List<Integer> rIndices = new ArrayList<>();
        	if (!rS.isEmpty()) {							
				for (DtRegistroCompleto dtr: rS) 
					rIndices.add(dtr.getId());
			}     
			try {
				if (rIndices.size() == 3) {
					icon.altaRegistro(rIndices);
					JOptionPane.showMessageDialog(this, "Registro canjeado exitosamente", "registro función", JOptionPane.INFORMATION_MESSAGE);
					cancelar();	
				} else {
					icon.altaRegistro(rIndices);
	        	...
	        	JOptionPane.showMessageDialog(this, e.getMessage(), "registro función", JOptionPane.ERROR_MESSAGE);
	        	limpiarFormulario();
	        }			
