<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<%@include file="/header.jsp" %>
<title>coronaTickets | Portal</title>
</head>
<body>
	<div class="fondo">	
		<div class="alert alert-dark" role="alert">
		  <div>
		    ${mensaje}
		  </div>
		</div>
 		<%if(detector.detectMobileQuick()){ %>
	 		<% if (tipo == null) { %>
			  <div class="offcanvas-body">
			    <main class="form-signin">
				  <form action="InicioSesion" name="inicio" method="post">
				    <img class="mb-4 center" src="imagenes/coronaticketsUY.jpg" alt="" width="220" height="149">
				    <h1 class="h3 mb-3 fw-normal">Ingrese sus datos</h1>
				    
				  	<div class="form-floating">
				      <input class="form-control" id="floatingInput" name="nickMail" required>
				      <label for="floatingInput">Nick</label>
				    </div>
				    <div class="form-floating">
				      <input type="password" class="form-control" id="floatingPassword" name="pass" required>
				      <label for="floatingPassword">Contraseña</label>
				    </div>
					<label class="checkbox text-white">
		  			  <input type="checkbox" value="remember-me" id="remember_me"> Recuerdame..
					</label>
		    
				    <button class="w-100 btn btn-lg btn-primary" type="submit">Ingresar</button>
				    <p class="mt-5 mb-3 text-muted">&copy; Developed by MCNC 2021</p>
				  </form>
				</main>
			  </div>
			
	 		<%}
	 		
	 		
	 	} %>
	 	<%if(detector.detectMobileQuick()){ %>
	 			<h1 style="font-size:5vw;">CORONATICKETS</h1>
	 		<%} else { %>
	 			<div class="logo-animado">
				  <h1 style="font-size:5vw;">CORONATICKETS</h1>		
					</div>
					 
	 		<%}%>
			  
	</div>
			
		
	<%@include file="/footer.jsp" %>
</body>
</html>
