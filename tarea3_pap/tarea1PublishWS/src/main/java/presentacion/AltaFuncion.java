...
public class AltaFuncion extends JInternalFrame {
	...
	protected void altaFuncionAceptarActionPerformed(ActionEvent e) throws NombreNullException, Exception{
		...
		if (fechaF.isBefore(fechaR))
			throw new Exception("No podes crear una función en el pasado");
		...
		DtFuncion dtf = new DtFuncion(nombreF,java.sql.Date.valueOf(fechaF), time,java.sql.Date.valueOf(fechaR), listaI); 
		try {
			this.icon.seleccionarEspectaculo(nombreE);
			this.icon.ingresarFuncion(dtf, null);			
			this.icon.chequearDisponibilidadNombreFuncion(nombreF);
			this.icon.altaFuncion();
        	JOptionPane.showMessageDialog(this, "Funcion creada exitosamente", "alta función", JOptionPane.INFORMATION_MESSAGE);
        	limpiarFormulario();
    		setVisible(false);	
		} catch (Exception e2) {
			JOptionPane.showMessageDialog(this, e2.getMessage(), "alta función", JOptionPane.ERROR_MESSAGE);	           		
		}		
	}
