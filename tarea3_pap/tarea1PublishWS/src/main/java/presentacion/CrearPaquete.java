...
public class CrearPaquete extends JInternalFrame {
	...
	private void aceptarCrearPaquete(ActionEvent e) throws PaqueteNoValidoException, NombreNullException, Exception{
		...
		if (fechaIni.isBefore(tmp) || fechaFin.isBefore(tmp))
			throw new Exception("No podes crear un paquete en el pasado");
		...
		if (icon.chequearDisponibilidadPaquete(nombre)) {
			...
			dtp = new DtPaquete(nombre, desc, java.sql.Date.valueOf(fechaIni),java.sql.Date.valueOf(fechaFin),java.sql.Date.valueOf(fechaR), descuento, l);
		} else {
			throw new PaqueteNoValidoException("Nombre inválido " + nombre + " ya existe en el sistema");
		}
		
		icon.ingresarDatosPaquete(dtp, null);
		icon.altaPaquete();		
