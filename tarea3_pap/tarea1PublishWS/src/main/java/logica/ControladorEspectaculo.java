...
public class ControladorEspectaculo implements IControladorEspectaculo {
	...
	public void altaEspectaculo() {		
		Espectaculo newE = new Espectaculo (dtesp.getNombre(), dtesp.getDescripcion(), dtesp.getURL(), dtesp.getDuracion(), dtesp.getCantMax(), dtesp.getCantMin(), dtesp.getCosto(), dtesp.getFechaReg().toInstant().atZone(ZoneId.systemDefault()).toLocalDate(), plat, artistaOrg, img);
		ManejadorEspectaculo mE = ManejadorEspectaculo.getInstancia();
