...
public class ControladorPaquete implements IControladorPaquete{
	...
	public void ingresarDatosPaquete(DtPaquete dtp, byte[] imagen) throws PaqueteNoValidoException {
		if (dtp.getNombre() == null)
			throw new PaqueteNoValidoException("El paquete ingresado no es valido");
		dtpaq = dtp;
		img = imagen;
	}

	public boolean chequearDisponibilidadPaquete(String nombre) throws PaqueteNoValidoException {
		ManejadorPaquete mP = ManejadorPaquete.getInstancia();
		Paquete p = mP.buscarPaquete(nombre);
		if (p == null) 
			return true; 
		else throw new PaqueteNoValidoException("El nombre " + nombre + " ya existe en el sistema");
	...
	public void altaPaquete() throws PaqueteNoValidoException {
		...
		if (p != null) {
			...
			Paquete newP = new Paquete(dtpaq.getNombre(), dtpaq.getDescripcion(), dtpaq.getFechaInicio(), dtpaq.getFechaFin(), dtpaq.getFechaRegistro(), dtpaq.getDescuento(), lE, img);
